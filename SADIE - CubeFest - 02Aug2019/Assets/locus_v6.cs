//This script is written for the Cube-fest performance. 
//New gestures are added.
//Added thumb to tip of the middle finger called as pinchm (as in pinch middle) command.
//Added one more optional command that calculates distance between tips of index fingers (left hand index finger to the right hand index finger), called as index2index or i2i.
//Date: August 2, 2019, 4:52 PM
...
public class locus_v6 : MonoBehaviour {
	...
	void Update()
	{
		...
		if (RI != null && RIJ != null && RKL!= null && RKR != null && RT != null && RWL != null && RM != null)
		{
			...
			if (Physics.Raycast(right_ray, out right_hit, 10000.0f, layer_mask) && Physics.Raycast(right_finger_ray, out right_finger_hit, 10000.0f, layer_mask))
			{
				...
				// calculate if a person is pointing or not
				double right_result=(Math.Acos((Math.Sin(right_finger_elev_angle*Math.PI/180)*Math.Sin(right_elev_angle*Math.PI/180))+(Math.Cos(right_finger_elev_angle*Math.PI/180)*Math.Cos(right_elev_angle*Math.PI/180)*Math.Cos(right_finger_azim_angle*Math.PI/180-right_azim_angle*Math.PI/180)))/Math.PI)*180;

				// right_azim_angle = (right_azim_angle + 90) % 360;
				sendString("right pointazel " + right_azim_angle + " " + right_elev_angle +  " " + right_finger_azim_angle + " " + right_finger_elev_angle + " " + right_result + "\n");
			...
			// Distance between right thumb and right middle finger
			Vector3 right_targetDir_middle_thumb_distance = RT.transform.position - RM.transform.position;
			float right_mag_middle_thumb_distance = Vector3.Magnitude(right_targetDir_middle_thumb_distance);
			...
			sendString("right pinchm " + right_mag_middle_thumb_distance + "\n");
		...
		if (LI != null && LIJ != null && LKL != null && LKR != null && LT != null && LWR != null && LM != null)
		{
			...
			if (Physics.Raycast(left_ray, out left_hit, 10000.0f, layer_mask) && Physics.Raycast(left_finger_ray, out left_finger_hit, 10000.0f, layer_mask))
			{
				...
				// calculate if a person is pointing or not
				double left_result=(Math.Acos((Math.Sin(left_finger_elev_angle*Math.PI/180)*Math.Sin(left_elev_angle*Math.PI/180))+(Math.Cos(left_finger_elev_angle*Math.PI/180)*Math.Cos(left_elev_angle*Math.PI/180)*Math.Cos(left_finger_azim_angle*Math.PI/180-left_azim_angle*Math.PI/180)))/Math.PI)*180;

				// send all five values
				sendString("left pointazel " + left_azim_angle + " " + left_elev_angle + " " + left_finger_azim_angle + " " + left_finger_elev_angle + " " + left_result + "\n");
			}
			...
			// Distance between left index finger and left middle finger
			Vector3 left_targetDir_middle_thumb_distance = LT.transform.position - LM.transform.position;
			float left_mag_middle_thumb_distance = Vector3.Magnitude(left_targetDir_middle_thumb_distance);
			...
			sendString("left pinchm " + left_mag_middle_thumb_distance + "\n");
		}
		...
		if (LI != null && RI != null) 
		{
			...
			// Distance between left index finger and right index finger
			Vector3 left_index2index = LI.transform.position - RI.transform.position;
			float left_mag_index2index_distance = Vector3.Magnitude(left_index2index);

			sendString("left i2i " + left_mag_index2index_distance + "\n");
