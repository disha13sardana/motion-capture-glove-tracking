...
public class locus_v2 : MonoBehaviour {
	...
	void Update()
	{
		...
		if (RI != null && RKL != null && RKR != null && RT != null && RWL != null && RIJ != null)
		{
			...
			if (Physics.Raycast(right_hand_ray, out right_hand_hit, 10000.0f, layer_mask))
			{
				...
				if (right_hand_impactDir_elev.y < right_hand_projDir.y) right_hand_elev_angle = 0 - right_hand_elev_angle;
			...
			if (Physics.Raycast(right_finger_ray, out right_finger_hit, 10000.0f, layer_mask))
			{
				// Subtract 6'9 height from it to get correct elevation
				//Vector3 right_hand_impactDir_elev = right_hand_hit.point - new Vector3(0f, 1.7526f, 0f);
				...
				//right_hand_azim_angle = (right_hand_azim_angle + 90) % 360;
				sendString("right fingerazel " + right_finger_azim_angle + " " + right_finger_elev_angle + "\n");
			}
		...
		if (LI != null && LKL != null && LKR != null && LT != null && LWR != null && LIJ != null)
		{
			...
			if (Physics.Raycast(left_hand_ray, out left_hand_hit, 10000.0f, layer_mask))
			{
				...
				if (left_hand_impactDir_elev.y < left_hand_projDir.y) left_hand_elev_angle = 0 - left_hand_elev_angle;
			...
			if (Physics.Raycast(left_finger_ray, out left_finger_hit, 10000.0f, layer_mask))
			{
				...
				sendString("left fingerazel " + left_finger_azim_angle + " " + left_finger_elev_angle + "\n");
			}
