// This the latest version at 1:41 pm, July 17, 2018
...
namespace QualisysRealTime.Unity{
    ...
    public class Binaural_v0: MonoBehaviour {
    // for right hand
    ...
    void Update () {
        ...
        // // Rigid body Binaural_Head
        // Binaural_Head = GameObject.Find("Binaural_Head");
        ...
        // if (Binaural_Head != null) {
        //     float binaural_head = Binaural_Head.transform.position.x;
            

        //     print("RB: " + binaural_head);
        ...
        if (body != null)
        {
            ...
            {
                print("Rigid Body Position: " + body.Position);
                print("Rigid Body Rotation: " + body.Rotation);
                print("Rigid Body Euler Angles: " + body.Rotation.eulerAngles);

                // print("Rigid Body Angle Roll: " + body.Rotation.x);
                // print("Rigid Body Angle Pitch: " + body.Rotation.y);
                // print("Rigid Body Angle Yaw: " + body.Rotation.z);

                // float roll = body.Rotation.eulerAngles.x;
                // float pitch = body.Rotation.eulerAngles.y;
                // float yaw = body.Rotation.eulerAngles.z;

                // print("RPY: "+ roll + pitch + yaw);

                // USR.transform.position = body.Position + PositionOffset;
                // if (USR.transform.parent) USR.transform.position += USR.transform.parent.position;

                // USR.transform.rotation = body.Rotation * Quaternion.Euler(EulerOffset);
                // if (USR.transform.parent) USR.transform.rotation *= USR.transform.parent.rotation;

                // planeNormal = body.Rotation*planeNormal;
                // planeXaxis = body.Rotation*planeXaxis;
        ...
        z = -Input.GetAxis("Horizontal") * Time.deltaTime * 3.0f;
        x = Input.GetAxis("Vertical") * Time.deltaTime * 3.0f;

        // USR.transform.Translate(x, 0, z);
        PositionOffset = new Vector3(x,0,z);
        USR.transform.position = USR.transform.position + PositionOffset;
        
        USR.transform.localEulerAngles = new Vector3(roll, 0, 0);
        USR.transform.Rotate(0, pitch, 0);
        
        Horizon.transform.Rotate(0, pitch, 0);
        PRJ.transform.localEulerAngles = new Vector3(roll, 0, 0);
    
        rotation = Quaternion.Euler(roll, 0, 0);
